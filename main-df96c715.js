function makeWorld() {
    ...
    for (var p = document.querySelectorAll(".matter-body"), b = [], M = 0, x = p.length; M < x; M++) {
        if ((p[M].classList.contains("hot") ? ((frA = 0.1), (oY = -40)) : ((frA = 0), (oY = 0)), p[M].classList.contains("strip")))
            ...
            var W = d.circle(n.centerX + Math.floor((Math.random() * n.width) / 2) - n.width / 4, 0, 75, { restitution: 0.3, friction: 2, frictionAir: 0, frictionStatic: 2, density: 100, angle: 2 * Math.random() - 1 });
        else if (p[M].classList.contains("page-nav-2"))
            var W = d.circle(n.centerX + Math.floor((Math.random() * n.width) / 2) - n.width / 4, 0, 50, { restitution: 0.3, friction: 2, frictionAir: 0, frictionStatic: 2, density: 100, angle: 2 * Math.random() - 1 });
        else if (p[M].classList.contains("page-nav-3"))
            var W = d.circle(n.centerX + Math.floor((Math.random() * n.width) / 2) - n.width / 4, 0, 37.5, { restitution: 0.3, friction: 2, frictionAir: 0, frictionStatic: 2, density: 100, angle: 2 * Math.random() - 1 });
